...
-   The name on the README.md is homework4. The link to the GitHub repository is https://github.com/YSun-umich/homework4.git
...
```{sql connection = con}
SELECT EXTRACT(YEAR FROM date) AS Year, COUNT(*) as Homicide_Arrests_Count 
FROM crime
WHERE primary_type = 'HOMICIDE' AND arrest = true 
GROUP BY Year 
ORDER BY Homicide_Arrests_Count DESC
```
...
FROM crime
WHERE EXTRACT(YEAR FROM date) = 2016 AND arrest = TRUE AND district = 11
GROUP BY primary_type
ORDER BY arrest_count DESC"
...
crime_tibble <- tbl(con, sql(crime_query))
```

Again, count the number of arrests grouped by `primary_type` of district 11 in year 2016, now using `dplyr` syntax.

```{r}
crime_data <- crime_tibble %>% filter(year == 2016 & arrest == TRUE & district == 11) %>% group_by(primary_type) %>% summarise(arrest_count = n()) %>% arrange(desc(arrest_count))
...
```{r}
crime_data_2 <- crime_tibble %>%  filter(district == 11, arrest == TRUE) %>% group_by(primary_type, year) %>% summarise(arrest_count = n()) %>%  arrange(desc(arrest_count))
...
```{r}
crime_data <- crime_data %>% collect()
crime_data_2 <- crime_data_2 %>% collect()
```
