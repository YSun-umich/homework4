...
crime_tib <- tbl(con, sql(crime))
```

Again, count the number of arrests grouped by `primary_type` of district 11 in year 2016, now using `dplyr` syntax.

```{r}
crime_data <- crime_tib %>% 
  filter(year == 2016 & arrest == TRUE & district == 11) %>% group_by(primary_type) %>% 
...
```{r}
arrest <- crime_tib %>%  filter(district == 11, arrest == TRUE) %>% group_by(primary_type, year) %>% summarise(arrest_count = n()) %>%  arrange(desc(arrest_count))
